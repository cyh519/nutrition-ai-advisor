...
# 2. 保存新的知识到知识库
def save_to_knowledge_base(filepath, new_knowledge):
    """将新的知识追加到txt文件"""
    with open(filepath, "a", encoding="utf-8") as f:
        f.write("\n" + new_knowledge)

# 3. 生成AI回答 (适配 Vercel 环境)
def generate_ai_response(user_question, knowledge_base):
    """调用 Replicate 的 Llama 3 API 生成回答"""
    # 从环境变量中获取 Replicate API 密钥
    replicate_api_key = os.getenv("REPLICATE_API_KEY")
    
    if not replicate_api_key:
        return "错误：Replicate API 密钥未设置。请在 Vercel 项目设置中添加环境变量 REPLICATE_API_KEY。"
